<?php
    ...
    if(isset($_POST['wishlist'])){
        ...
        $cid = $_SESSION['id'];
        $sql = "INSERT INTO `wishlist` (`email`, `cid`, `dt`) VALUES ('$email', '$cid', current_timestamp())";
        $res = $conn->query($sql);
        if(!$res){
    ...
    if(isset($_POST['buylist'])){
        $email = $_SESSION['email'];
        $cid = $_SESSION['id'];
        $sql = "INSERT INTO `buylist` (`cid`, `email`, `bought`, `dt`) VALUES ('$cid', '$email', '1', current_timestamp())";
        $res = $conn->query($sql);
        if(!$res){
...
<body>
    ...
    <nav>
        <div class="container nav_container">
            ...
            <ul class="nav_menu">
                <li class="active"><a href="./Courses.php" aria-current="page" te>Courses</a></li>
            ...
            if ($res->num_rows>0) {
                while ($row =  $res->fetch_assoc()) {
                    ...
                    <div class="container container__course">
                        <div class="course__left" data-aos="fade-right">
                                <div class="course__image" data-aos="fade-right">
                                    <img src="data:image/jpg;charset=utf8;base64,<?=base64_encode($row['img'])?>" alt="">
                                </div>
                                <div class="cos">
                                    <form action="./Course.php" method="post">
                                        <button type="submit" class="cbtn" name="wishlist">Add to Wishlist<i class="uil uil-heart mx-2"></i></button>
                                    </form>
                                    <form action="./Course.php" method="post">
                                        <button type="submit" class="cbtn" name="buylist">Buy Now<i class="uil uil-shopping-cart-alt"></i></button>
                                    </form>
                                </div>
                            </div>
                            <div class="course__right" data-aos="fade-left">
                                <h3 class="py-2">Course Description</h3>
                                <!-- <div class="">lorem50</div> -->
                                <div><?=$row['desc']?></div>
                            </div>
                        </div>
                        <div class="course__below" data-aos="fade-up">
